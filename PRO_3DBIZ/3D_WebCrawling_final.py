...
# todo 3D_WebCrawling_final_ad 대체 되었고. 터널링을 위해서 3D_WebCrawling_final_ad_real 사용해야 한다.
"""
...
########################################################################################## 딕셔너리
DIC_INFO = {
    "101": "대한상공회의소-경제자료-보도자료",
    "102": "대한상공회의소-경제자료-브리프 인포",
    "103": "대한상공회의소-경제자료-온라인 세미나",
    "104": "대한상공회의소-경제자료-경제컬럼",
    "105": "대한상공회의소-산업소식-경제정책정보",
    "106": "대한상공회의소-산업소식-유관기관소식",
    "107": "대한상공회의소-산업소식-기업뉴스",
    "108": "한국경제인협회-뉴스자료-보도자료 및 발표문",
    "109": "한국경제인협회-이슈앤포커스-FKI인사이트",
}
...
# 날짜 제한을 두자.
def date_collect(x):
    return (datetime.date.today() + datetime.timedelta(days=-x)).strftime(
        "%Y%m%d"
    )  # 사이트마다 타입이 달라서 숫자8개로 통일
...
def maria_db_insert(result_list, orign_key):
    if len(result_list) > 0:
        ...
        try:
            ...
            if len(df) > 0:
                df.to_sql(
                    "tn_tdb_crawling", conn, if_exists="append", index=False
                )  # db insert
                conn.commit()
                print(f"{len(df)} Insert success")

                # 데이터베이스 입력건수를 데이터베이스에 입력
                query = f"select ORIGIN, TDAY from tn_tdb_collectinfo where ORIGIN='{df.loc[0, 'ORIGIN']}' AND TDAY='{df.loc[0, 'TDAY']}'"
                result_df = pd.read_sql_query(query, conn)
                if len(result_df) == 0:
                    alist = [
                        [
                    ...
                    df_in.to_sql(
                        "tn_tdb_collectinfo", conn, if_exists="append", index=False
                    )
                    conn.commit()
                    ...
                    try:
                        ...
                        if detail_data:
                            ...
                            if link_text1 > 0 and link_text2 > 0:
                                link_text = link_text[
                                    int(link_text1) + 2 : int(link_text2) + 3
                                ]
...
def crawling_103():
    ...
    driver.get(
        "https://www.korcham.net/nCham/Service/Economy/appl/OnlineSeminarList.asp"
    )
    time.sleep(3)
            ...
            if tap_drive:
                ...
                for _ in range(0, 30):
                    try:
                        ...
                        if tap_num == 1:
                            caption_link = (
                                "/html/body/div/section[1]/figure/figcaption/a"
                            )
                            caption_data = driver.find_element(By.XPATH, caption_link)

                            # 등록일자를 구해서 오늘(새벽)을 기준으로 이전 3일치만 구하자.
                            reg_date = (
                                f"/html/body/div/section[1]/figure/figcaption/a/span[2]"
                            )
                            reg_date = driver.find_element(By.XPATH, reg_date).text
                            ...
                            if caption_data:
                                ...
                                if len(caption_list) > 0:
                                    ...
                                    if caption_title_index > 0:
                                        caption_title_text = caption_title_text[
                                            0:caption_title_index
                                        ]
                                    result_list.append(
                                        [
                                            orign_key,
...
def crawling_108():
    ...
    for _ in range(0, 2):
        try:
            tap_xpath_link = (
                f"/html/body/div[4]/div[2]/div[2]/div/div[1]/ul/li[{tap_num}]/a"
            )
            tap_drive = driver.find_element(By.XPATH, tap_xpath_link)
...
def crawling_109():
    ...
    for _ in range(0, 2):
        try:
            tap_xpath_link = (
                f"/html/body/div[3]/div[2]/div[2]/div/div[1]/ul/li[{tap_num}]/a"
            )
            tap_drive = driver.find_element(By.XPATH, tap_xpath_link)
            ...
            if tap_drive:
                ...
                for _ in range(0, 30):
                    try:
                        # 찾고자 하는 속성이 포함된 엘리먼트를 찾고
                        detail_xpath_link = (
                            f"/html/body/div[3]/div[2]/div[2]/div/ul/li[{detail_num}]"
                        )
                        detail_data = driver.find_element(By.XPATH, detail_xpath_link)
