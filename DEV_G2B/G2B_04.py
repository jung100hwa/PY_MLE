...
# 변수선언
xmlUrl = 'http://apis.data.go.kr/1230000/PrvtBidNtceService/getPrvtBidPblancListInfoServcPPSSrch'
...
if len(gdf) == 0:
    ...
    strQ = "ntcenm.str.contains('시스템|포털|홈페이지|정보|db|DB', regex=True)"
    strQ = "ntcenm.str.contains('시스템', regex=True)"
    gdf2 = gdf.query(strQ)
    result = gdf2


    columns = {'bidntceno':'입찰공고번호','nticedt':'게시일시','opengdt':'개찰일시','ntcenm': '공고명','ntcedivnm':'공고구분명','ntceinsttnm':'공고기관명',
               'bidmethdnm':'입찰방식명','cntrctmthdnm':'계약방법명','sucsfbidmthdnm':'낙찰방법명'}
    ...
    SU_Pandas_MO.SU_MO_To_excel(result, "C:\\work\\03.제안작성\\"+fname+"_민간 공고현황.xlsx")
