...
# while strbeforeday <= '201706010001':
...
if len(gdf) == 0:
    ...
    # strQ = "bidntcenm.str.contains('시스템|포털|홈페이지|정보|db|DB', regex=True)"
    # result = gdf.query(strQ)
    
    result = gdf


    columns = {'bidntceno':'공고번호','bidntcenm':'사업명','prtcptcnum':'참가업체수','bidwinnrnm':'낙찰업체','bidwinnrbizno':'사업자등록번호',
               'sucsfbidamt':'낙찰금액','sucsfbidrate':'낙찰률','rgstdt':'등록일시'}
    result = result[columns]
    result.rename(columns=columns, inplace=True)

    # 중복값을 제거하자, 조달청 api가 조금 명확하지 않음
    result = result.drop_duplicates()
    SU_Pandas_MO.SU_MO_To_excel(result, "C:\\work\\03.제안작성\\낙찰업체분석_"+ comnumber +".xlsx")
