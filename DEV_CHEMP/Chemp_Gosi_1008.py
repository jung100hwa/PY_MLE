...
# 엑셀파일 열기
wb = op.load_workbook(G_ExFilePos + "승인고시_0128.xlsx")
ws = wb.worksheets[0]
...
for rowitem in range(2, ws.max_row + 1):
    ...
    if strCasno:
        ...
        # 아래는 2021년 07부터 고유번호 없음-8 이였던것이 고유번호-6으로 바뀜
        # if strCasno == '고유번호 없음-6' or strCasno == '7681-52-9':
        #     strCasno = '7681-52-9'
        if strCasno == '고유번호 없음-7' or strCasno == '7778-54-3':
            strCasno = '7778-54-3'
    else:
        ws.cell(row=rowitem, column=13, value='CAS_NO가 존재하지 않음')
        continue
    ...
    # 제품유형코드를 정제, 언제부터인가 아래 유형코드로 전달해주고 있음
    ...
    if ptcode:
        ...
        ws.cell(row=rowitem, column=13, value='유형코드가 존재하지 않음')
        continue
    ...
    if strMstid:
        ...
        # 업체가 제품유형협의체에 있는지 조사, 사업자등록번호, 선임여부, 국외제조자
        ...
        if strResult:
            ...
            if strAplcnttype == '4':
                ...
                cursor.execute("""SELECT MST_ID FROM TN_EBM_COOPERATION_MAP_T WHERE MST_ID='%s' AND BIZ_NO='%s' AND 
                REPLACE(AGENT_NAME,' ','')='%s'""" % (strMstid, strBizno, strCnnmtnImsi))
                strResult1 = cursor.fetchone()

                # 국외제조자는 같은데 수입자를 추가한 경우, 선임사실신고증번호가 다를 수 있다.
                if strResult1:
                    cursor.execute("""UPDATE TN_EBM_COOPERATION_MAP_T SET AGENT_YN='%s' WHERE MST_ID='%s' AND BIZ_NO='%s' AND 
                    REPLACE(AGENT_NAME,' ','')='%s'""" % (strManageno, strMstid, strBizno, strCnnmtnImsi))
                    connection.commit()

                 # 국외제조자가 다른 경우는 무조건 입력
                else:
                    strCnnmtn = strCnnmtn.replace("'", "' || chr(39) || '")
                    ...
                    """ % (strMstid, strBizno, '009', 'Y', 'Y', 'N', 'N', '', strTime, strWrid, strTime, strWrid, 'Y', strManageno, strCnnmtn))
                    connection.commit()
...
wb.save(G_ExFilePos + "승인고시_0128.xlsx")
wb.close()
