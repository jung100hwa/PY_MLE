# 권혜림 연구원 수정보완 자진취하 등 반려건까지 추출
# 권혜림 연구원 수입면제 통계 뽑는 것
...
wb = op.load_workbook(G_ExFilePos + "20220107_권혜림수입요건면제2.xlsx")
ws = wb.worksheets[0]
...
FROM TN_IRN_EXEMP TA WHERE TA.EXEMP_CN='01'
    """)
...
for row in cursor:
    ...
    cursor2.execute("""SELECT TO_CHAR(SUBMIT_DATE ,'YYYY-MM-DD'), (SELECT CODE_NM FROM COMTCCMMNDETAILCODE WHERE CODE_ID='IRN007' AND CODE=TA.PRO_STS)
     FROM TN_IRN_EXEMP_MOD TA WHERE EXEMP_SEQ=%s""" %(strNo))
    ...
    for row in cursor2:
        for col in range(0,len(cursor2.description)):
            ...
            if va == 'None':
                va = ''
            for item in range(0,len(strListValue)):
                ws.cell(row=rowcount, column=item+1, value=strListValue[item])

            if col == 0:
            	ws.cell(row=rowcount, column=col+1, value=va)
            if col ==1:
                ws.cell(row=rowcount, column=col+5, value=va)
        rowcount = rowcount + 1


wb.save(G_ExFilePos + "20220107_권혜림수입요건면제2.xlsx")
wb.close()