...
def Bad_Return_Rows(rows, itable, ipar, mainkey):
    ...
    for itemrow in rows:
        ...
        valueDoc       = idf.iloc[0,0]      # 문서명
        valueMaincode  = idf.iloc[0,1]      # 메인항목 코드
        valueSubcode   = idf.iloc[0,2]      # 서브항목 코드
        valueColname   = idf.iloc[0,3]      # 컬럼영
        valueFormat    = idf.iloc[0,4]      # 값형태
        valueTaborText = idf.iloc[0,5]      # 테이블인지 문장인지
        valueTabindex  = idf.iloc[0,6]      # 테이블인경우 테이블 인덱스
        valueColindex  = idf.iloc[0,7]      # 테이블인경우 컬럼 인덱스

        if valueFormat:

            if valueFormat == '1':      # 텍스트값
                itable[0].rows[count].cells[1].text = str(itemrow[17])

            if valueFormat == '2':      # 코드값
                codevalue = Bad_Return_SqlkeyvalueBAD(
                    str(itemrow[11]), str(itemrow[17]))
                itable[0].rows[count].cells[1].text = codevalue

            if valueFormat == '3':      # 범위값
                vlist = Bad_Return_Split(str(itemrow[17]), "##")
                itable[0].rows[count].cells[2].text = vlist[0]
                itable[0].rows[count].cells[4].text = vlist[1]

            if valueFormat == '4':      # 이미지 첨부파일(구조식 같은)
            ...
            # 테이블에 일반적인 반복 형태
            if valueFormat[0:1] == '6' and gubunnum == 'RG3171':
                imsicount          = rowcount + 2
