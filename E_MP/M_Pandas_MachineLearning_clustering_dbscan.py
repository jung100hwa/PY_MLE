'''
군집 알고리즘인 dbscan를 이욯
예측 값은 없음, 설명변수(컬럼)이 늘어나면 늘어날수록 클러스터 개수가 늘어남
또한 클러스트 개수가 늘어나면 노이즈가 늘어남
공간 밀집도를 기준으로 구분
'''
...
G_AptSample = G_ExFilePos + '\\ODT\\Pandas\\'

# 서울시내 중학교 진학률 데이터셋 (출처: 교육???)
df = pd.read_excel(G_AptSample + '2016_middle_shcool_graduates_report.xlsx', header=0)
...
# # 지도를 html 파일로 저장하기
mschool_map.save(G_AptSample + 'seoul_mschool_location.html')
...
########################################### test
# 유니크한 값 출력
# print(df['location'].unique())      # unique values list
# print(df['location'].unique().shape)
# print(df['location'].unique().size) # unique count
# print(df['location'])
...
# # 전체 df에 널값이 있는지 체크, 하나라도 있으면 true
...
# 지도를 html 파일로 저장하기
cluster_map.save(G_AptSample+ 'seoul_mschool_cluster.html')
...
# 설명유형 즉 독립변수가 많아지면 클러스터 개수가 증가한다.
# 또한 노이즈 개수도 늘어난다.
...
# 지도를 html 파일로 저장하기
cluster2_map.save(G_AptSample + 'seoul_mschool_cluster2.html')
...
# 지도를 html 파일로 저장하기
cluster3_map.save(G_AptSample + 'seoul_mschool_cluster3.html')