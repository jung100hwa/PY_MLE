...
try:
    G_Connection = cx_Oracle.connect("scott/tiger@192.168.56.1:1521/XE")
    ...
    if len(rows) > 0:
        for row in rows:
            ...
            if len(strURL) > 10 :
                ...
                retxtfile = SM.SU_FOLD_MONITORING2(G_TRNS_HWPFOLDER, G_TXT_SALVEFOLDER, G_Cursor, G_Connection, G_SPLIT_GUBUN, 1)

                if len(retxtfile) > 0:

                    # 추출한 파일 경로 삽입
                    sql = """INSERT INTO PR_T_MONFILE VALUES('%s','%s','%s')""" % (strAC, retxtfile, strDate)
                    G_Cursor.execute(sql)
